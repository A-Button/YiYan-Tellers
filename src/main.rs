...
fn read_sentences() -> Result<Vec<Sentence>, Box<dyn std::error::Error>> {
...
fn format_sentence(sentence: &Sentence) -> String {
...
fn main() {
    match read_sentences() {
        Ok(sentences) => {
            ...
            if let Some(random_sentence) = sentences.choose(&mut rng) {
                println!("{}", format_sentence(random_sentence));
            }
        }
        Err(e) => {
            eprintln!("错误: {}", e);
